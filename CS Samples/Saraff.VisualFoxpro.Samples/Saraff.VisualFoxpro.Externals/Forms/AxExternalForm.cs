...
namespace Saraff.VisualFoxpro.Externals.Forms {
    ...
    public abstract class AxExternalForm<T, TResult>:VfpExternalForm<TResult> where T:VfpApplicationControl {

        protected AxExternalForm() {
        }

        protected override object InvokeFormCore(string name, object[] args) {
            var _args=new object[Math.Max(args.Length, 1)+1];

            var _type=typeof(T);
            _args[0]=string.Format("{0}!{1}", Path.GetFileName(_type.Assembly.Location), _type.FullName);
            ...
            return base.InvokeFormCore(name, _args);
        }
