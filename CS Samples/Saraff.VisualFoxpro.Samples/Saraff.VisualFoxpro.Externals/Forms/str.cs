...
namespace Saraff.VisualFoxpro.Externals.Forms {
    ...
    public sealed class str:VfpExternalForm<int> {

        public bool Invoke(out int result) {
            return base.Invoke(out result, 1);
        }

        public bool Invoke(out int result,bool isShowDisposedSubdivisions) {
            return base.Invoke(out result,1,isShowDisposedSubdivisions);
        }
        ...
        protected override bool IsSuccess(object value, out int result) {
            result=-1;
            return value!=null&&int.TryParse(value.ToString(), out result)&&result!=-1;
        }
    }
}
