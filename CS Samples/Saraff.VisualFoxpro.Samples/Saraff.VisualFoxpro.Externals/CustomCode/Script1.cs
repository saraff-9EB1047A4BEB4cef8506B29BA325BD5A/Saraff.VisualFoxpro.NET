...
namespace Saraff.VisualFoxpro.Externals.CustomCode {

    public sealed class Script1:VfpExternalCode {

        protected override string VfpCode {
            get {
                return @"
LPARAMETERS oFRHost as Object
LOCAL NDATE,EDATE,itog_str

DO GETINTERVAL WITH NDATE, EDATE, 3,.f.,.t.,.t.,.f.
IF !EMPTY(NDATE) AND !EMPTY(EDATE)
	oFRHost.SetParameterValue('start_date',NDATE)
	oFRHost.SetParameterValue('end_date',EDATE)
ELSE
	RETURN .F.
ENDIF

itog_str=GETSPISOK(.T.,NDATE,EDATE)
IF TYPE('itog_str')=='C' THEN
	IF itog_str<>'-1' THEN
		oFRHost.SetParameterValue('subdivisions',itog_str)
	ELSE
		RETURN .F.
	ENDIF
ELSE
