...
using System.Text;
using Saraff.VisualFoxpro.Externals.Forms;

namespace Saraff.VisualFoxpro.Samples {

    public sealed class SampleControlExternalForm:AxExternalForm<SampleControl,int> {

        protected override bool IsSuccess(object value, out int result) {
            result=0;
            return (value is int) && int.TryParse(value.ToString(), out result);
        }
    }
}
