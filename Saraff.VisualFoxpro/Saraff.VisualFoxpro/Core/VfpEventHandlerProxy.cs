...
namespace Saraff.VisualFoxpro.Core {
    ...
    internal sealed class VfpEventHandlerProxy {

        private VfpEventHandlerProxy() {
        }

        public static VfpEventHandlerProxy Create(AxHostComponent container) {
            return new VfpEventHandlerProxy {
                Container=container
            };
        }

        public void Send(EventArgs args) {
            var _method=this.Container.CreateMethodDescriptor("Send");
            _method.AddParam(args, ParamType.Unknown);
            this.Container.PerformMethod(_method);
        }

        public AxHostComponent Container {
            get;
            private set;
        }
    }
