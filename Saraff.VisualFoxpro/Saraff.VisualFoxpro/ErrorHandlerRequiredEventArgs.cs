...
namespace Saraff.VisualFoxpro {
    ...
    public sealed class ErrorHandlerRequiredEventArgs:EventArgs {

        public ErrorHandlerRequiredEventArgs(Exception ex) {
            this.ExceptionType=ex.GetType().Name;
            var _message=string.Empty;
            for(var _ex=ex; _ex!=null; _ex=_ex.InnerException) {
                _message+=string.Format("{1}: {2}{0}{3}{0}{0}", Environment.NewLine, _ex.GetType().Name, _ex.Message, _ex.StackTrace);
            }
            this.Message=_message;
        }

        public ErrorHandlerRequiredEventArgs(string message, string type) {
